...
const ROLES: Role[] = ["management", "staff", "facility", "repricer", "pending", "suspended"];
              ...
              <tr key={f.id} className={idx % 2 ? "bg-surface/40" : ""}>
                ...
                <td className="td">
                  <input
                    ...
                    step="1"
                    min="0"
                    defaultValue={f.php_floor ?? ""}
                    onBlur={(e) => {
                      const v = e.target.value.trim() === "" ? null : parseFloat(e.target.value);
                      const next = v == null || isNaN(v) ? null : v;
                      if (next !== (f.php_floor ?? null)) save(f, { php_floor: next });
                    }}
